...
class Config:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            ...
            cls._instance.args = None  # Initialize the 'args' field
...
def logger(info, model, optimizer, args=None):
...
def main():
    ...
    if args.standard_rating:
        if args.data_name in ["flixster", "ml_10m"]:  # original 0.5, 1, ..., 5
            rating_map = {
                x: int(math.ceil(x)) for x in np.arange(0.5, 5.01, 0.5).tolist()
            }
    ...
    if args.visualize:
        ...
        print(args)
        logger(eval_info, None, None, args=args)
