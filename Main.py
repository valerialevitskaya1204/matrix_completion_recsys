...
# Строим полный путь к файлу Main.py
    main_file = os.path.join(current_dir, 'Main.py')

# Проверяем наличие файла и копируем
    if os.path.exists(main_file):
        copy(main_file, args.res_dir)
    else:
...
# Получаем текущую директорию, где находится Main.py
    current_dir = os.path.dirname(os.path.abspath(__file__))

# Полные пути к файлам
    util_functions_path = os.path.join(current_dir, 'util_functions.py')
    models_path = os.path.join(current_dir, 'models.py')
    train_eval_path = os.path.join(current_dir, 'train_eval.py')

# Проверяем наличие файлов перед копированием
    if os.path.exists(util_functions_path):
        copy(util_functions_path, args.res_dir)
    else:
        print(f"File {util_functions_path} not found!")

    if os.path.exists(models_path):
        copy(models_path, args.res_dir)
    else:
        print(f"File {models_path} not found!")

    if os.path.exists(train_eval_path):
        copy(train_eval_path, args.res_dir)
    else:
        print(f"File {train_eval_path} not found!")
