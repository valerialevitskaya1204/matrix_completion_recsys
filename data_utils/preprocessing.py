...
def globally_normalize_bipartite_adjacency(adjacencies, verbose=False, symmetric=True):
    ...
    if symmetric:
        adj_norm = [
            degree_u_inv_sqrt_mat.dot(adj).dot(degree_v_inv_sqrt_mat)
...
def create_trainvaltest_split(
    ...
    if datasplit_from_file and os.path.isfile(datasplit_path):
        ...
        num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features = (
            load_data(dataset, seed=seed, verbose=verbose)
        )
    ...
    if dataset == "ml_100k":
        ...
        with open(movie_file, "rb") as f:
            ...
            movie_df = pd.read_csv(
                movie_file, sep=sep, header=None, names=movie_headers, encoding=encoding
            )
        ...
        for _, row in users_df.iterrows():
            ...
            if u_id in u_dict.keys():
                # age
                u_features[u_dict[u_id], 0] = row["age"] / np.float64(age_max)
                # gender
